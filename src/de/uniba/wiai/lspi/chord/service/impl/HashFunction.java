...
public final class HashFunction {
	...
	public static HashFunction getHashFunction(int m) {
	 ...
	 *            Message digest for calculating hash values.
	 */
	private HashFunction(MessageDigest digest1, int m) {
		if (digest1 == null) {
			throw new NullPointerException("Parameter may not be null!");
		}
		this.messageDigest = digest1;
	...
	public final ID createID(byte[] testBytes) {
		...
		synchronized (this.messageDigest) {
			...
			byte[] compressed = new byte[m];
			for (int j = 0; j < m; j++) {
				byte temp = result[j];
				for (int k = 1; k < 20/m; k++) {
					temp = (byte) (temp ^ result[(j+k)%result.length]);
				}
				compressed[j] = temp;
			}
			return new ID(compressed);
		}
	}
